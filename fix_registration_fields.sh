...
echo "🔧 회원가입 필드 오류 수정 중..."

# Django 시리얼라이저 파일 경로
SERIALIZER_FILE="backend/apps/accounts/serializers/auth.py"

# 백업 생성
cp "$SERIALIZER_FILE" "${SERIALIZER_FILE}.backup"

# 기존 extra_kwargs 부분을 찾아서 교체
cat > temp_serializer.py << 'EOF'
"""
인증 관련 시리얼라이저
...
# 파일 교체
mv temp_serializer.py "$SERIALIZER_FILE"

echo "✅ 시리얼라이저 수정 완료!"

# Django 서버 재시작 (개발 서버가 실행 중이라면)
echo "🔄 Django 서버 재시작 중..."
cd backend
python manage.py collectstatic --noinput > /dev/null 2>&1 || true

echo "✅ 수정 완료! 이제 회원가입을 다시 시도해보세요."
echo ""
echo "📝 변경사항:"
echo "   - first_name, last_name 필드를 선택사항으로 변경"
echo "   - 빈 값 허용 설정 추가"
echo "   - 상세한 로깅 추가"
echo ""
echo "🚀 이제 이름을 입력하지 않아도 회원가입이 가능합니다!"
